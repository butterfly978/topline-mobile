<template>
  <div>
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
      <van-cell v-for="item in comments" :key="item.com_id.toString()">
        <div slot="icon">
        ...
        <div slot="default">
          <van-button :icon="item.is_liking ? 'like' : 'like-o'" size="mini" plain @click="handleLikeComment(item)">赞 {{ item.like_count }}</van-button>
        </div>
        <div slot="label">
          <p>{{ item.content }}</p>
          <p>
