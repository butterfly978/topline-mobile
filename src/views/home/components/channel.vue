...
<!--
  ...
  lazy-renser 关闭懒加载
 -->
  <van-popup :value="value" @input="$emit('input', $event)" position="bottom" get-container="body" :style="{ height: '95%' }">
    <!-- 我的频道 -->
    <div class="channel">
      ...
      <van-grid class="channel-content" :gutter="10" clickable>
        <van-grid-item v-for="item in recommendChannels" :key="item.id" @click="handleAddChannel(item)">
          <div class="info">
    ...
    <div class="channel">
      ...
      <van-grid class="channel-content" :gutter="10" clickable>
        <van-grid-item v-for="(item, index) in userChannels" :key="item.id" @click="handleUserChannelClick(item, index)">
  ...
  methods: {
    ...
    async handleAddChannel (item) {
      ...
      if (user) {
        ...
        // 如果没有登录，则添加到本地存储
        // 没有就创建，有的直接覆盖
        // 注意：本地存储数据无法像js数据变量去修改，要想改变只能完全重写
