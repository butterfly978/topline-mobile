<template>
  <div>
    ...
    <home-channel v-model="isChannelShow" :user-channels="channels" :active-index="activeChannelIndex" />
    <!-- /频道组件 -->
  </div>
</template>
...
export default {
  ...
  wacth: {
    ...
    async '$store.state.user' () {
      ...
      // 而且上拉加载更多的onLoad没有主动触发
      // 我们这里可以手动的触发上拉加载更多的onLoad
      // 提示：只需要将当前激活频道的上拉loading设置为true，它会自动调用自己的onLoad函数
      // 注意：这里肯定是有别的东西影响了，没有自动调用onLoad
      this.activeChannel.upLoading = true
      // 正常的话上面设置loading之后，组件会自动去onLoad
      // 这里它没有自己onLoad，那我们就自己手动的onLoad一下
      this.onLoad()
    }
  },
  async created () {
  ...
  methods: {
    ...
    async loadChannels () {
      ...
      if (user) {
        ...
        // 如果没有登录
        // 判断是否有本地存储的频道列表
        const loadChannels = JSON.parses(window.localStorage.getItem('channels'))
        // 如果有，则使用
        if (loadChannels) {
